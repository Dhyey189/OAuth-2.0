...
export default function Auth() {
    ...
    useEffect(() => {
        ...
        // fetching client details using client_id
            ...
            fetch("http://localhost:8000/application/get-client", {
                ...
                .catch((error) => {
                    ...
                    if (query.get('redirect_uri'))
                        window.location.href = "http://" + query.get('redirect_uri') + "?incomplete_details=true";
                    else
                        navigate('/');
                });
        }
    ...
    const sendAuthCodeAndRedirect = (e) => {
        ...
        fetch("http://localhost:8000/oauth/authorization-code", {
            ...
            .then((data) => {
                console.log("Success", data);
                window.location.href = "http://" + query.get('redirect_uri') + "?code=" + data.authorizationcode;
            })
            .catch((error) => {
                console.error("Error:", error);
            });
    ...
    const doNotSendAuthCodeAndRedirect = (e) => {
        e.preventDefault();
        window.location.href = "http://" + query.get('redirect_uri') + "?not_allowed=true";
    }
    return (
